...
export default function ProductList() {
  ...
  useEffect(() => {
    fetch("https://stuffy-backend-api.onrender.com/api/products")
      .then(res => res.json())
      .then(data => {
        // Guard: if backend returns { error: "..." } instead of [] don't crash
    ...
    // Listen for AI search results from Header App (cross-MFE via CustomEvent)
    const handleAIResult = (e) => {
      setAiMatches(e.detail.matches); // null = clear filter
    };
  ...
  if (loading) return <div style={{ color: "var(--text-muted)", fontSize: '1rem', textAlign: 'center', padding: '40px' }}>Loading products...</div>;

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
         <h3 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, color: 'var(--text-main)' }}>Products</h3>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "30px" }}>
        {products.map((p) => {
          ...
          // Determine if this product is an AI match
            ...
            <div key={p.id} className="ds-glass-card" style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto', paddingTop: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {isFlashing && <span style={{ fontSize: '0.75rem', color: '#ef4444', fontWeight: '700', marginBottom: '4px' }}>Price drop!</span>}
